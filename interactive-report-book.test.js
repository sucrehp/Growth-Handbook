...
assert.match(child, /function getCartoonTagIcon\(label, group = ''\)/);
assert.match(child, /function renderIconTag\(label, group = ''\)/);
assert.match(child, /function renderRecordSource\(source, compact = false\)/);
assert.match(child, /家长补充 · 已审核/);
assert.match(child, /机构老师记录/);
assert.match(child, /PARENT CONTRIBUTION · 家长填写区/);
assert.match(child, /const timelineRecords = publishedRecords[\s\S]*record\.type === 'MILESTONE'/);
assert.match(child, /renderTimeline\(timelineRecords, profile\.photos, c\.birthday\)/);
assert.match(child, /const evidence = \(item\.evidence \|\| \[\]\)\.find/);
assert.match(child, /const imageUrl = evidence/);
